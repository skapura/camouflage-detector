...
train1 = Image.open('img/cadpat_marpat_saturate.jpg')
w, h = train1.size
Xtrain = list()
Ytrain = list()
Xtest = list()
...
for x in range(0, 10):
    for y in range(0, 10):
        img = train1.crop((x, y, x + 100, y + 100))
...
w, msehist = train(dftrain, Ytrain, [dftrain.shape[1], dftrain.shape[1]*3, 20, 5, 1], 100, eta=2.0, decay=0.01, stoperr=0.01)
...
for i in range(0, len(dffull)):
    ...
    print(yhat)
    ypred = 0 if yhat < 0.9 else 1
    if ypred == 1:
        print(testcoords[i])
        dr.rectangle((testcoords[i], (testcoords[i][0]+100, testcoords[i][1]+100)), fill=(255, 0, 0, 128))
imgout.save('img/cadpat_marpat_output.jpg')
