# Final project
# Nicholas Skapura
# Camouflage detection and assessment using MLPs
...
for i in range(0, 100):
    img = Image.open('img/train/nature_b_' + str(i) + '.jpg')
    l = list(img.getdata())
...
for trainIndex, testIndex in kf.split(dfx.loc[:, :dfx.shape[1]], dfx.loc[:, dfx.shape[1]-1]):
    ...
    trainX = pcatrans.fit_transform(trainX)
    ...
    #w, msehist = train(trainX, trainY, [xdim, xdim * 3, 5, 1], 10, eta=3.0, decay=0.01, stoperr=0.01)
    w, msehist = train(trainX, trainY, [xdim, xdim * 3, 5, 1], 100, eta=1.0, decay=0.01, stoperr=0.01)
