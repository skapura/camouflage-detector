...
test1 = Image.open('img/pencott_train_saturation.jpg')
...
#for x in range(0, 10):
#    for y in range(0, 10):
#        img = train1.crop((x, y, x + width, y + height))
...
for x in range(0, w - width, width):
    for y in range(0, h - height, height):
        img = test1.crop((x, y, x + width, y + height))
        #img = ImageOps.grayscale(img)
        l = list(img.getdata())
        l = list(chain.from_iterable(l))
...
#w, msehist = train(dftrain, Ytrain, [dftrain.shape[1], 10, 3, 10, dftrain.shape[1]], 100, eta=3.0, decay=0.01, stoperr=0.01)
w, msehist = train(dftrain, dftrain, [dftrain.shape[1], int(dftrain.shape[1] / 2), dftrain.shape[1]], 2000, eta=3.0, decay=0.01, stoperr=0.01)
...
for i in range(0, len(Ytest)):
    ...
    #    e += (yhat[j] - dftest[i][j]) ** 2
    #e /= len(yhat)
    print(str(Ytest[i]) + ', ' + str(e))
    #print('a')
    ...
    #else:
    #    numwrong += 1
    #print(str(Ytrain[i][0]) + ":" + str(ypred) + ", " + str(yhat))print(numright)
print(numwrong)
...
imgout = Image.open('img/pencott_train.jpg')
dr = ImageDraw.Draw(imgout, 'RGBA')
for i in range(0, len(dffull)):
    yhat = forward(dffull[i], w)
    print(dffull[i])
    print(yhat)
    print(mse(dffull[i], yhat))
    #ypred = 0 if yhat < 0.6 else 1
    #if ypred == 1:
    if mse(dffull[i], yhat) > 0.8:
        print(testcoords[i])
        dr.rectangle((testcoords[i], (testcoords[i][0]+width, testcoords[i][1]+height)), fill=(255, 0, 0, 128))
imgout.save('img/pencott_train_output3.jpg')
